...
func main() {
	...
	fmt.Fprintf(f, `<script type="text/javascript" id="commodities">
	globalThis.importedCommodities = `)
	if err := encoder.Encode(coin.Commodities); err != nil {
		fmt.Fprint(os.Stderr, err.Error())
	}
	fmt.Fprintln(f, "\n</script>")
	fmt.Fprintf(os.Stderr, "Commodities: %d\n", len(coin.Commodities))
	fmt.Fprintf(f, `<script type="text/javascript" id="prices">
	globalThis.importedPrices = `)
	if err := encoder.Encode(coin.Prices); err != nil {
		fmt.Fprint(os.Stderr, err.Error())
	}
	fmt.Fprintln(f, "\n</script>")
	fmt.Fprintf(os.Stderr, "Prices: %d\n", len(coin.Prices))
	fmt.Fprintf(f, `<script type="text/javascript" id="accounts">
	globalThis.importedAccounts = `)
	if err := encoder.Encode(coin.AccountsByName); err != nil {
		fmt.Fprintf(os.Stderr, err.Error())
	}
	fmt.Fprintln(f, "\n</script>")
	fmt.Fprintf(os.Stderr, "Accounts: %d\n", len(coin.AccountsByName))
	fmt.Fprintf(f, `<script type="text/javascript" id="transactions">
	globalThis.importedTransactions = `)
