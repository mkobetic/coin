...
class Account {
  ...
  shortenedName(child: Account): string {
...
type predeclaredGlobals = {
  importedCommodities: Record<
    string,
...
function loadCommodities() {
  for (const impCommodity of Object.values(
    (globalThis as unknown as predeclaredGlobals).importedCommodities
  )) {
  ...
  const prices = (globalThis as unknown as predeclaredGlobals).importedPrices;
  if (prices) {
    for (const imported of prices) {
...
function loadAccounts() {
  for (const impAccount of Object.values(
    (globalThis as unknown as predeclaredGlobals).importedAccounts
  )) {
  ...
  for (const impTransaction of Object.values(
    (globalThis as unknown as predeclaredGlobals).importedTransactions
  )) {
...
function addIncludeSubAccountsInput(
  ...
  container
    ...
    .on("change", (e, d) => {
      ...
      State.Register.ShowSubAccounts = input.checked;
      State.SelectedView(containerSelector, account);
    })
    .attr("id", "includeSubAccounts")
    .attr("type", "checkbox")
    .property("checked", State.Register.ShowSubAccounts);
}

function addSubAccountMaxInput(containerSelector: string, account: Account) {
  const container = d3.select(containerSelector);
  container
    .append("label")
  ...
  container
    ...
    .on("change", (e, d) => {
      const input = e.currentTarget as HTMLInputElement;
      State.Register.AggregatedSubAccountMax = parseInt(input.value);
      State.SelectedView(containerSelector, account);
    })
    .attr("id", "subAccountMax")
    .attr("type", "number")
    .property("value", State.Register.AggregatedSubAccountMax);
}
...
function addAggregateInput(
  ...
  aggregate.on("change", (e, d) => {
    ...
    State.Register.Aggregate = selected as keyof typeof Aggregation;
    State.SelectedView(containerSelector, account);
  });
  let data = Object.keys(Aggregation).filter(
    (k) => opts.includeNone || k != "None"
  );
  if (opts.includeNone && selected == "None") {
    selected = data[0];
    State.Register.Aggregate = selected as keyof typeof Aggregation;
  }
  aggregate
    ...
    .data(data)
    .join("option")
    .property("selected", (v) => v == selected)
    .property("value", (v) => v)
    .text((v) => v);
}
...
function viewRegister(
  ...
  addAggregateInput(containerSelector, State.Register.Aggregate, account);
  if (State.Register.ShowSubAccounts && State.Register.Aggregate != "None")
    addSubAccountMaxInput(containerSelector, account);

  const groupKey = Aggregation[State.Register.Aggregate];
  if (groupKey) {
  ...
  table
    ...
    .data((p) => {
      ...
      return [
        [dateToString(p.transaction.posted), "date"],
        [p.transaction.description, "text"],
        [account.shortenedName(p.account), "account"],
        [p.transaction.other(p).account, "account"],
        [p.quantity, "amount"],
        [Amount.clone(total), "amount"],
      ];
...
function viewChart(
  ...
  const groupKey = Aggregation[State.Register.Aggregate] as d3.TimeInterval;
  const dates = groupKey.range(State.StartDate, State.EndDate);
  const maxAccounts = State.Register.AggregatedSubAccountMax;
  const groups = groupWithSubAccounts(account, groupKey, maxAccounts, {
    negated: opts.negated,
  });
  const labelFromAccount = (a: Account | undefined) =>
    a ? account.shortenedName(a) : "Other";
  const labels = groups.map((gs) => labelFromAccount(gs.account));
  ...
  y.domain(dates.map((d) => dateToString(d)));
  x.domain([0, max]).nice();
  ...
  layer
    ...
    .enter()
    .append("rect")
    .attr("y", (d) => y(dateToString(d.date)) ?? 0)
  ...
  layer
    ...
    .attr("x", (d) => x((d.offset ?? 0) + (d.width ?? 0) - 2))
    .attr("y", (d) => (y(dateToString(d.date)) ?? 0) + (y.bandwidth() * 3) / 4);
  ...
  var w = (x.domain()[1] - 10) / labels.length;
...
function addViewSelect(account: Account) {
  ...
  d3.select(ViewSelect)
    .on("change", (e) => {
      ...
      State.SelectedView =
        selectedViews[selectedView as keyof typeof selectedViews];
      updateView(account);
    })
    .selectAll("option")
...
function addAccountList(account: Account) {
  d3.select(AccountList)
    ...
    .text((d) => d.fullName)
    .on("click", (e: Event) =>
      updateView((e.currentTarget as liWithAccount).__data__)
    );
}

function updateAccounts(account: Account) {
...
function initializeUI() {
  ...
  d3.select(EndDateInput)
    ...
    .on("change", (e) => {
      ...
      updateView(State.SelectedAccount);
    });
